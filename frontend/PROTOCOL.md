...
| `record_started` | Record | `id, kind:"tool"\|"skill"\|"spawn"\|"sleep"\|"module"\|"agent"\|"error"`, `name`, `summary?` | 机器过程开始(braille 转轮) |
...
3. verb 的 kind 映射:invoke_tool/list_tools/show_tool → `tool`,invoke_skill/list_skills/show_skill → `skill`,spawn → `spawn`,sleep → `sleep`;未知 verb 落 `verb` 兜底(前端 glyph '•')。
